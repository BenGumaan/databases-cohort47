...
async function findEpisodesExercises(client) {
  ...
  // Finding the title of episode 2 in season 2
  ...
  // Finding the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  ...
  // Finding all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]
  ...
  // Finding all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
...
async function updateEpisodeExercises(client) {
  ...
  // Updating episode 13 in season 30 to have a title of "BLUE RIDGE FALLS".
  ...
  // Updating all of the documents in the collection that have `BUSHES` in the elements array to have `BUSH`.
  const updateResult2 = await collection.updateMany(
    { elements: "BUSHES" },
    { $set: { "elements.$": "BUSH" } }
  );
  console.log(`Ran a command to update all the BUSHES to BUSH and it updated ${updateResult2.modifiedCount} episodes`);
...
async function deleteEpisodeExercise(client) {
  ...
  const db = client.db("databaseWeek3"); 
  const collection = db.collection("bob_ross_episodes");
  // Deleting episode 14 in season 31
